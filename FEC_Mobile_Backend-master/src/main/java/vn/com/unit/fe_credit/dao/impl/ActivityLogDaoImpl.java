...
@Repository
public class ActivityLogDaoImpl extends GenericDAOImpl<ActivityLog, Long>
	...
	public long sumChart(List<String> list, String FromDay, String ToDay,
		...
		hql.append("select count(*) from ActivityLog ac ");
		String id = null;
	...
	public List<ActivityLog> activityLogsCustomer(long type, long customerId) {
		...
		sql.append("select ac.activityId as activityId ,ac.logCode as logCode ,ac.logDate as logDate,ac.descritption as descritption,ac.userType as userType,ac.type as type,ac.refeId as refeId,ac.userId as userId,ac.customerId  as customerId from ActivityLog ac where ac.customerId="
				+ customerId
				+ " and ac.userId="
		...
		query.setResultTransformer(Transformers.aliasToBean(ActivityLog.class));
		return query.list();
	}
}
