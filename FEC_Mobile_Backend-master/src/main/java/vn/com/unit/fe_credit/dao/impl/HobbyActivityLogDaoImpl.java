...
@Repository
public class HobbyActivityLogDaoImpl extends GenericDAOImpl<HobbyActivityLog, Long>
		implements HobbyActivityLogDao {
	@Autowired
	...
	public long sumChart(List<String> list, String FromDay, String ToDay,
		...
		hql.append("select count(*) from HobbyActivityLog ac ");
		String id = null;
	...
	public List<HobbyActivityLog> hobbyActivityLogsCustomer(long type, long customerId) {
		...
		sql.append("select ac.activityId as activityId ,ac.logCode as logCode ,ac.logDate as logDate,ac.descritption as descritption,ac.userType as userType,ac.type as type,ac.refeId as refeId,ac.userId as userId,ac.customerId  as customerId from HobbyActivityLog ac where ac.customerId="
				+ customerId
				+ " and ac.userId="
		...
		query.setResultTransformer(Transformers.aliasToBean(HobbyActivityLog.class));
		return query.list();
	}
}
