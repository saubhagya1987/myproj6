...
@Repository
public class MessageActivityLogDaoImpl extends GenericDAOImpl<MessageActivityLog, Long>
		implements MessageActivityLogDao {
	@Autowired
	...
	public long sumChart(List<String> list, String FromDay, String ToDay,
		...
		hql.append("select count(*) from MessageActivityLog ac ");
		String id = null;
	...
	public List<MessageActivityLog> messageActivityLogsCustomer(long type, long Id) {
		...
		sql.append("select ac.activityId as activityId ,ac.logCode as logCode ,ac.logDate as logDate,ac.descritption as descritption,ac.userType as userType,ac.type as type,ac.refeId as refeId,ac.userId as userId,ac.Id  as Id from MessageActivityLog ac where ac.Id="
				+ Id
				+ " and ac.userId="
				+ userProfile.getAccount().getId()
				+ " or ac.userId is null and ac.Id="
				+ Id
		...
		query.setResultTransformer(Transformers.aliasToBean(MessageActivityLog.class));
		return query.list();
	}
}
