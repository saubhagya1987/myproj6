...
<%@ attribute name="documentFrom" required="false" rtexprvalue="true" type="java.lang.Integer" description="document form"%>
...
<c:if test="${empty documentFrom }"><c:set var="documentFrom" value="0"></c:set> </c:if>
						...
						<div class="title_table row-fluid">
							...
							<div class="span6 add_table">
								...
								<span style="float: right;padding-right: 10px"> <input
									type="button" class="btn btn-info" value="${payment_attach }" class="draft" id="btnAddFile"/>
								</span>
								</c:if>
							</div>
						</div>
...
<div id="viewModal" class="modal hide fade" tabindex="-1" role="dialog"
...
function openPopup(id){
	if(id==null)
	  ajaxUrl= '${url}/system/documentinfo/ajax?fragments=body&u_id='+$.now()+'&entity.documentSource=${documentSource}&entity.referenceId=${referenceId}&entity.referenceNo=${referenceNo}&visible=${visible}&count='+count+'&checkPR=${checkPR}&levelPR=${levelPR}';
	else
	  ajaxUrl= '${url}/system/documentinfo/ajax?fragments=body&u_id='+$.now()+'&entity.documentSource=${documentSource}&entity.referenceId=${referenceId}&entity.referenceNo=${referenceNo}&visible=${visible}&id='+id+'&count='+count+'&checkPR=${checkPR}&levelPR=${levelPR}';
	
	$.ajax({url:ajaxUrl,success:function(result){
		count++;
	    $("#myModal").html(result);
	    $("#myModal #documentFromId").val('${documentFrom}');
	    $("#myModal").modal('show');
	  }});
}
...
function openPopupView(id){
	if(id==null)
	  ajaxUrl= '${url}/system/documentinfo/ajaxview?fragments=body&u_id='+$.now()+'&entity.documentSource=${documentSource}&entity.referenceId=${referenceId}&visible=${visible}';
	else
	  ajaxUrl= '${url}/system/documentinfo/ajaxview?fragments=body&u_id='+$.now()+'&entity.documentSource=${documentSource}&entity.referenceId=${referenceId}&visible=${visible}&id='+id;
	$.ajax({url:ajaxUrl,success:function(result){
...
var ajaxUrl= '${url}/system/documentinfo/ajaxlist?listDocumentArrayId='+listDocumentTempId+'&fragments=body&u_id='+$.now()+'&entity.documentSource=${documentSource}&entity.referenceId=${referenceId}&visible=${visible}&checkPR=${checkPR}&levelPR=${levelPR}';
$.ajax({url:ajaxUrl,success:function(result){
	$("#listDocumentTempId").val($(result).find("#listDocumentArrayId").val());
...
function deleteDoc(id){
	...
	confirmMessage('${msg_deleteQuestion}',function(result){
		if(result){
			var ajaxUrl='${url}/system/documentinfo/ajaxdelete?listDocumentArrayId='+listDocumentTempId+'&fragments=body&u_id='+$.now()+'&entity.documentSource=${documentSource}&entity.referenceId=${referenceId}&visible=${visible}&id='+id+'&checkPR=${checkPR}&levelPR=${levelPR}';
			$.ajax({url:ajaxUrl,success:function(result){
			    $("#uploaded-files").html(result);
			  }});
		}
