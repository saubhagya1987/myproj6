...
<%@ attribute name="placeholder" required="false" rtexprvalue="true" type="java.lang.String" description="placeholder"%>
<%@ attribute name="uppercase" required="false" rtexprvalue="true" type="java.lang.Boolean" description="Upper case javascript"%>
...
<c:if test="${empty uppercase }"><c:set var="uppercase" value="false"></c:set> </c:if>
...
<c:when test="${isGrid}">
				...
				<form:input path="${path }" maxlength="${maxlength}" cssClass="${cssInput}" autocomplete="false" disabled="${disable }" value="${value}" placeholder="${ placeholder}" readonly="${readonly }" id="${id }" style="width: ${csswidth}px" />
				<c:if test="${disable}">
...
<div class="control-group">
			...
			<div class="controls">
				...
				<form:input path="${path }" maxlength="${maxlength}" cssClass="${cssInput}" cssErrorClass="${cssInput} errorfeborder" autocomplete="false" disabled="${disable }" placeholder="${ placeholder}" readonly="${readonly }" id="${id }"  />
				<c:if test="${disable}">
				<form:hidden path="${path}"/>
				</c:if>
				<c:set var="error_clss" value="${fn:replace(path,'.','_') }"></c:set>
...
<c:if test="${uppercase}">
	<script type="text/javascript">
		$('#${id}').change(function(eventObject){
