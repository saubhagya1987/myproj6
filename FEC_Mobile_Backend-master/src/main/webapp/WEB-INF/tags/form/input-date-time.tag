...
<c:if test="${empty disable}"> <c:set var="disable" value="false"></c:set></c:if>
<c:if test="${empty format}"> <c:set var="format" value="${sessionScope.formatDate} hh:mm"></c:set></c:if>
<c:if test="${empty csswidth }"><c:set var="csswidth" value="110"></c:set> </c:if>
...
<c:when test="${isGrid}">
				...
				<span class="help-inline ${error_clss}_msg">
					...
					<form:errors path="${path}" cssClass="error" cssStyle="padding-left:25px;"></form:errors>
				</span>
...
<div class="control-group">
			...
			<div class="controls">
				<div class="input-append date" id="${id}" data-date="${value}" 
													data-date-format="${ format}">
													<input class="span12" size="10" type="text"   
