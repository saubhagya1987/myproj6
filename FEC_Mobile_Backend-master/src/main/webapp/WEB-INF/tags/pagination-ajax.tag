...
<c:if test="${bean.total > bean.limit }">
	<ul>
		<c:if test="${bean.page gt 1 }">
			<li><a href="javascript:submitAndSetHiddenVal_Ajax('${formId}',{'page':'${bean.page-1 }','maxPage':'${maxPage}'},'','${ajaxUrl }','${divId }')">
		...
		<c:forEach var="i" items="${bean.listPage }">
			...
			<li><a href="javascript:submitAndSetHiddenVal_Ajax('${formId}',{'page':'${i }','maxPage':'${maxPage}'},'','${ajaxUrl }','${divId }')" class="${bean.page eq i?'paging-active':'' }">${i }</a></li>
		</c:forEach>
		<c:if test="${bean.page < bean.totalPage }">
			<li><a href="javascript:submitAndSetHiddenVal_Ajax('${formId}',{'page':'${bean.page+1 }','maxPage':'${maxPage}'},'','${ajaxUrl }','${divId }')"><spring:message code="pagination.next"></spring:message></a></li>
		</c:if>
	</ul>
</c:if>
</div>
