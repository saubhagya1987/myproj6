...
pathAnim = {
	...
	init: function (elem, fromD, toD) {
		...
		if (shift <= end.length / numPareprocurement && start.length === end.length) {
			while (shift--) {
				end = [].concat(end).splice(0, numPareprocurement).concat(end);
			}
		}
		...
		if (start.length) {
			...
			while (start.length < endLength) {
				...
				//bezier && sixify(start);
				slice = [].concat(start).splice(start.length - numPareprocurement, numPareprocurement);
				if (bezier) { // disable first control point
					slice[numPareprocurement - 6] = slice[numPareprocurement - 2];
					slice[numPareprocurement - 5] = slice[numPareprocurement - 1];
				}
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		 ...
		 * @param {Object} el
		 * @param {Object} pareprocurement
		 * @param {Object} options jQuery-like animation options: duration, easing, callback
		 */
		animate: function (el, pareprocurement, options) {
			var $el = $(el);
			if (!el.style) {
				el.style = {}; // #1881
			}
			if (pareprocurement.d) {
				el.toD = pareprocurement.d; // keep the array form for paths, used in $.fx.step.d
				pareprocurement.d = 1; // because in jQuery, animating to an array has a different meaning
			}
	
			$el.stop();
			if (pareprocurement.opacity !== UNDEFINED && el.attr) {
				pareprocurement.opacity += 'px'; // force jQuery to use same logic as width and height (#2161)
			}
			$el.animate(pareprocurement, options);
...
SVGElement.prototype = {
	 ...
	 */
	animate: function (pareprocurement, options, complete) {
		...
		if (animOptions) {
			...
			animate(this, pareprocurement, animOptions);
		} else {
			this.attr(pareprocurement);
			if (complete) {
				complete();
			}
		}
