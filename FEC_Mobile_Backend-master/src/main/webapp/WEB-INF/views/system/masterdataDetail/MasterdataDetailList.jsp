...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#previewDetailPopup').bind('hide', function () {
			if($("#reloadyesorno").val()=="1"){
				document.location.href = "${url}/system/masterdataDetail/list";
				/* $("#reloadyesorno").val(""); */
			}
		 });
	});
	...
	function showTextVal(showPopup,id) {
		$.ajax({
			url : '${url}/system/masterdataDetail/MasterdataDetail',
			data : {
				addedIds : $('#addedIds').val(),
				codeSearchPopup : $('#codeSearchPopup').val(),
				masterdataDetailId : id
			},
			dataType : 'html',
			type : 'GET',
			cache : false,
	...
	function deleteAction(id) {
		confirmMessage('${msg_deleteQuestion}', function(result) {
			if (result) {
				document.location.href = "${url}/system/masterdataDetail/delete?entity.masterdatadetailId="
						+ id;
			}
		});
	}
	...
	function blackAccountMasterdata() {
		document.location.href = "${url}/system/masterdata/list";
	}
</script>
<!-- start title -->
...
<div class="container-fluid">
		...
		<spring:message var="menu_admin" code="menu.admin"></spring:message>
			...
			<spring:message var="menu_system_masterdataDetail" code="menu.system.masterdataDetail"></spring:message>
			<spring:message var="menu_system_masterdata_list" code="menu.system.masterdataDetailList"></spring:message>
		<h4 style="padding: 8px 0 0 10px;">
				<a onclick="back()" class="Color_back"><c:out value="${menu_admin }"></c:out></a>
				<span> > </span>
				<a onclick="blackAccountMasterdata()" class="Color_back"><c:out value="${menu_system_masterdata }"></c:out></a>
				<span> > </span>
				<a onclick="blackAccount()" class="Color_back"><c:out value="${menu_system_masterdataDetail }"></c:out></a>
				<span> > </span>
...
<div class="container-fluid unit_bg_content">
		...
		<div class="row-fluid">
			...
			<div class="span6 title_h2 title">
				<spring:message var="msg_List" code="menu.system.masterdataDetailList"></spring:message>
				<h2>${msg_List}</h2>
				...
				<div class="input-area">
					<div class="row-fluid search">
						<div class="span12 input-append" style="background: White">
							<spring:message var="msg_branch" code="search.masterdata"></spring:message>
							...
							<button type="button"
								onclick="submitAndSetHiddenVal('search_form_masterdata',{'page':'1','maxPage':'5'})"
				...
				<table class="table table-bordered table-hover out-tbl">
					...
					<tr>
						...
						<tr>
							...
							<spring:message var="msg_master" code="menu.master"></spring:message>
					...
					<tbody>
						...
						<c:forEach var="_masterdata" items="${bean.listResult }" varStatus="i">
							<tr id="tr${index}">
								...
								<td><c:out value="${_masterdata.masterdata.name }" /></td>
								...
								<a onclick="showTextVal(true,${_masterdata.masterdatadetailId})"title="${icon_edit }"> <i
										class="bms-icon-edit"></i></a>
								<a href="javascript:void(0)"
									onclick="deleteAction('${_masterdata.masterdatadetailId }')"
