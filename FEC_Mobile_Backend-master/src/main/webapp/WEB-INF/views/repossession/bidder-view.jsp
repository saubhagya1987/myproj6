		...
		<div class="row-fluid">
			<div class="span6">
				...
				<h4 style="padding: 8px 0 0 10px; margin:0px 0px 0px">
					...
					<a onclick="backRepossessionData()" class="Color_back">Repossession</a><span> &gt; </span>
					<span class="Color_back">Bidder</span>
				</h4>
			</div>
                 ...
                 <!--change html 25-11-16-->
                    ...
                    <div class="table-responsive ">
  						<table class="table reposs table-hover">
   									<thead>
  									 <tr>
	  									 <th><spring:message code="customer.contractId" /></th>
	  									 <th><spring:message code="customer.bidderName" /></th>
	  									 <th><spring:message code="customer.amount" /></th>
	  									 <th><spring:message code="customer.entryDate" /></th>
	  									 <th><spring:message code="customer.status" /></th>
        								...
        								<tbody>
        								<%-- <p>aaaaaaaaaaaaa<c:out value="${fn:length(bean.contractTerminationDtoList)}"/></p>  --%>
        								<c:choose>
        								 <c:when test="${fn:length(bean.bidderList ) <= 0}">
        								<%--  <p><c:out value="${fn:length(beans.contractTerminationDtoList)}"/></p> --%> 
        							...
        							<c:otherwise>
        								<c:forEach var="contractTerminationList" items="${bean.bidderList }"
											varStatus="i">
											<%-- <p><c:out value="${fn:length(beans.contractTerminationDtoList)}"/></p>  --%>
											<tr>
										
												<%-- <td class="w50_stt text_center">${(bean.page-1)*bean.limit+i.index+1}</td> --%>
												<td><c:out value="${contractTerminationList.applId}"></c:out></td>
												<td><c:out value="${contractTerminationList.bidderName}"></c:out></td>
												<td><c:out value="${contractTerminationList.amount }"></c:out></td>
												<td><c:out value="${contractTerminationList.entryDate }"></c:out></td>												
												<td><c:out value="${contractTerminationList.status }"></c:out></td>
												...
												<td>
			                                        ...
			                                        <a href="${url}/contract/repossession/bidderDetails?contractId=${contractTerminationList.applId}&id=${contractTerminationList.id}"><img src="${url}/static/images/view.png" alt=""/></a><br/>View</div>
			                                        </li>
			                                        <%-- <sec:authorize access="hasAnyRole('CA','CS&A','R016','R017','R018', 'R019')"> --%>
			                                        <li>
			                                        <div class="change-status-collection">
			                                        <a href="${url}/contract/repossession/updateStatusBidder?contractId=${contractTerminationList.applId}&repossessionId=${contractTerminationList.repossessionId}&bidder=${contractTerminationList.id}"><img src="${url}/static/images/status.png" alt=""/></a><br/>Status<br/>Change
			                                        </div>
			                                        </li>
			                                       <%--  </sec:authorize> --%>
			                                        
			                                        <%-- <sec:authorize access="hasAnyRole('FS')"> --%>
			                                        <li>
			                                        ...
			                                        <input type="hidden" id="reassign_url_${i.index}" value="${url}/contract/termination/reassign?contractId=${contractTerminationList.applId}&repossessionId=${contractTerminationList.repossessionId}&nextApproverLevel=${contractTerminationList.nextApproverLevel}"/>
