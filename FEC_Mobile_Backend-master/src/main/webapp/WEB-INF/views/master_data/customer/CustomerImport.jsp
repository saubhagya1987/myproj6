...
<script type="text/javascript">
	...
	function downloadImportCustomer(){
		$("#search_form").attr("action", "${url}/master_data/customer/downloadImportCustomer");
		$("#search_form").submit();
	}
	...
	function importExcel(){
		...
		$("#search_form").attr("action", "${url}/master_data/customer/import");
		$("#search_form").submit();
	}
</script>
<style>
...
<div class="container-fluid unit_bg_content">
		...
		<ext:messages bean="${bean}"></ext:messages>
			...
			<div class="span6 title_h2 title" style="margin-left: 5px;">
				<h2  >
					<spring:message var="list_title" code="menu.customer.import"></spring:message>
				...
				<div class="input-area">
					...
					<div class="row-fluid" style="margin-top: 15px">
						<div class="span12" style="text-align: center;">
							<spring:message var="downloadTemp" code="customer.importFile"></spring:message>
							<c:out value="${downloadTemp }"></c:out>
							<a href="javascript:void(0);" onclick="downloadImportCustomer();"><img
				...
				<table class="table table-bordered table-hover out-tbl">
					<thead>
						...
						<tr>
							...
							<th>
								<spring:message code="customer.field.first.name" />
							</th>
							<th>
								<spring:message code="customer.field.middle.name" />
							</th>
							<th>
							<spring:message code="customer.field.last.name" />
							</th>
							<th>
							<spring:message code="customer.field.card.id.no" />
							</th>
							<th><spring:message code="customer.field.cell.phone" /> </th>
							<th><spring:message code="customer.field.oldCustomer" /> </th>
							
						</tr>
					</thead>
					<tbody>
						<c:forEach var="_item" items="${bean.customerImportLst }"	varStatus="i">
							<tr>
								<td class="w50_stt text_center">${(bean.page-1)*bean.limit+i.index+1}</td>
								
								</td>
								<td>
									<c:out value="${_item.firstName }"></c:out>
								</td>
								<td>
									<c:out value="${_item.lastName }"></c:out>
								</td>
								<td>
									<c:out value="${_item.middleName }"></c:out>
								</td>
								<td>
									<c:out value="${_item.idCardNumber }"></c:out>
								</td>
								<td> 
									<c:out value="${_item.cellPhone }"></c:out>
								</td>
								<td> 
									<c:out value="${_item.oldUserId }"></c:out>
								</td>
							</tr>
						</c:forEach>
						<tr>
